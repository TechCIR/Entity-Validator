...
public class LongValueInValidator extends BaseFieldValidator implements FieldValidator {

    public LongValueInValidator(Field field, Object entity) throws IllegalAccessException {
        super(field, entity);
    }
    @Override
    public void validate(List<ValidationError> errors) throws UnsupportedFieldException, IllegalAccessException {
        if (null != value) {
            ...
            LongValueIn intValueIn = field.getAnnotation(LongValueIn.class);
            boolean valueShouldIn = intValueIn.shouldIn();
            boolean isContain = Arrays.stream(intValueIn.values())
                .anyMatch(value -> Long.valueOf(value).equals(valInLong));
